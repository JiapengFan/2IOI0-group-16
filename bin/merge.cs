// Dafny program merge.dfy compiled into C#
...
// Dafny 3.0.0.30203
// Command Line Options: c:\Users\Windows10\Downloads\merge.dfy /verifyAllModules /compile:3 /spillTargetCode:1 /out:bin\merge
// merge.dfy

function sortedRange(a: array<int>, low: nat, high: nat): bool
...
method Merge(a: array<int>, low: nat, mid: nat, high: nat)
  requires 0 <= low <= mid <= high <= a.Length
  requires sortedRange(a, low, mid)
  requires sortedRange(a, mid, high)
  modifies a
  ensures forall i: int | 0 <= i < low || high <= i < a.Length :: a[i] == old(a[i])
  decreases a, low, mid, high
{
  var L1: array<int>, L2: array<int> := new int[mid - low], new int[high - mid];
  var p1, p2, p3 := 0, 0, 0;
  while p1 < L1.Length
    invariant 0 <= p1 <= L1.Length
    invariant forall m: int | 0 <= m < low :: a[m] == old(a[m])
    invariant forall m: int | high <= m < a.Length :: a[m] == old(a[m])
    decreases L1.Length - p1
  {
    L1[p1] := a[low + p1];
    p1 := p1 + 1;
  }
  while p2 < L2.Length
    invariant 0 <= p2 <= L2.Length
    invariant forall m: int | 0 <= m < low :: a[m] == old(a[m])
    invariant forall m: int | high <= m < a.Length :: a[m] == old(a[m])
    decreases L2.Length - p2
  {
    L2[p2] := a[mid + p2];
    p2 := p2 + 1;
  }
  p1, p2 := 0, 0;
  while p3 < high - low
    invariant 0 <= p1 <= L1.Length && 0 <= p2 <= L2.Length
    invariant 0 <= low <= p3 + low <= high <= a.Length
    invariant forall i: int | 0 <= i < low :: a[i] == old(a[i])
    invariant forall i: int | high <= i < a.Length :: a[i] == old(a[i])
    decreases high - low - p3
  {
...
namespace _module {
  ...
  public partial class __default {
    public static void Merge(BigInteger[] a, BigInteger low, BigInteger mid, BigInteger high)
    {
      BigInteger[] _63_L1;
      BigInteger[] _64_L2;
      BigInteger[] _nw0 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked((mid) - (low), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      BigInteger[] _nw1 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked((high) - (mid), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      BigInteger[] _rhs0 = _nw0;
      BigInteger[] _rhs1 = _nw1;
      _63_L1 = _rhs0;
      _64_L2 = _rhs1;
      BigInteger _65_p1;
      BigInteger _66_p2;
      ...
      _66_p2 = _rhs3;
      _67_p3 = _rhs4;
      while ((_65_p1) < (new BigInteger((_63_L1).Length))) {
        (_63_L1)[(int)((_65_p1))] = (a)[(int)((low) + (_65_p1))];
        _65_p1 = (_65_p1) + (BigInteger.One);
      }
      while ((_66_p2) < (new BigInteger((_64_L2).Length))) {
        (_64_L2)[(int)((_66_p2))] = (a)[(int)((mid) + (_66_p2))];
        _66_p2 = (_66_p2) + (BigInteger.One);
      }
      BigInteger _rhs5 = BigInteger.Zero;
      BigInteger _rhs6 = BigInteger.Zero;
      _65_p1 = _rhs5;
      _66_p2 = _rhs6;
      while ((_67_p3) < ((high) - (low))) {
        if (((_65_p1) == (new BigInteger((_63_L1).Length))) && ((_66_p2) == (new BigInteger((_64_L2).Length)))) {
          goto after_0;
        } else if ((_65_p1) == (new BigInteger((_63_L1).Length))) {
          var _index0 = (low) + (_67_p3);
          (a)[(int)(_index0)] = (_64_L2)[(int)(_66_p2)];
          _66_p2 = (_66_p2) + (BigInteger.One);
        } else if ((_66_p2) == (new BigInteger((_64_L2).Length))) {
          var _index1 = (low) + (_67_p3);
          (a)[(int)(_index1)] = (_63_L1)[(int)(_65_p1)];
          _65_p1 = (_65_p1) + (BigInteger.One);
        } else {
          if (((_63_L1)[(int)(_65_p1)]) < ((_64_L2)[(int)(_66_p2)])) {
            var _index2 = (low) + (_67_p3);
            (a)[(int)(_index2)] = (_63_L1)[(int)(_65_p1)];
            _65_p1 = (_65_p1) + (BigInteger.One);
          } else {
            var _index3 = (low) + (_67_p3);
            (a)[(int)(_index3)] = (_64_L2)[(int)(_66_p2)];
            _66_p2 = (_66_p2) + (BigInteger.One);
          }
        }
        _67_p3 = (_67_p3) + (BigInteger.One);
      }
    after_0: ;
    }
    ...
    {
      BigInteger[] _68_a;
      BigInteger[] _nw2 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(new BigInteger(2), "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      _nw2[(int)(0)] = BigInteger.Zero;
      _nw2[(int)(1)] = BigInteger.One;
      _68_a = _nw2;
    }
  }
